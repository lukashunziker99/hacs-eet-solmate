...
async def async_setup_entry(hass, entry, async_add_entities):
    ...
    async_add_entities([SolMateSwitch(coordinator)])
...
class SolMateSwitch(SwitchEntity):
    ...
    async def async_turn_on(self):
        if self.coordinator.writer:
            await self.coordinator.writer.write("force_charge", True)

    async def async_turn_off(self):
        if self.coordinator.writer:
            await self.coordinator.writer.write("force_charge", False)
